...
var _tent = require("@tentjs/tent");
const { div, button } = (0, _tent.tags);
const Counter = {
    state: {
        count: 0
    },
...
(0, _tent.mount)(document.getElementById("counter"), Counter);
const GetStarted = {
    view: ()=>button("Get Started", {
            onclick,
            class: "cta"
        })
};
...
(0, _tent.mount)(document.getElementById("get-started"), GetStarted);

},{"@tentjs/tent":"2WGbh"}],"2WGbh":[function(require,module,exports) {
...
function createTag(context) {
    ...
    if (Array.isArray(children)) children.forEach((c)=>elm.append(Array.isArray(c) ? createTag(c) : c));
...
function walker(oldNode, newNode) {
    const nc = Array.from(newNode.children);
    if (oldNode.children.length < nc.length) nc.forEach((x, index)=>{
        if (!oldNode.children[index]) oldNode.append(addAttributes(x.cloneNode(true), x));
    });
    Array.from(oldNode.children).forEach((oChild, index)=>{
        ...
        if (oChild.children.length < nChild.children.length) {
            ...
            Array.from(nChild.children).forEach((ncc, index)=>{
                if (!occ[index]) oChild.append(addAttributes(ncc.cloneNode(true), ncc));
            });
        }
        ...
        Array.from(nChild.attributes).forEach((attr)=>{
            if (oChild.getAttribute(attr.name) !== attr.value) oChild.setAttribute(attr.name, attr.value);
        });
        // Remove attributes that are not present in the new node
        Array.from(oChild.attributes).forEach((attr)=>{
            if (!nChild.hasAttribute(attr.name)) oChild.removeAttribute(attr.name);
        });
        // Replace text content if it's different and the element has no children
        if (oChild.textContent !== nChild.textContent && nChild.children.length === 0 && oChild.children.length === 0) oChild.textContent = nChild.textContent;
        walker(oChild, nChild);
    });
}
function addAttributes(clone, node) {
    if (!clone.$tent && !node.$tent) return clone;
    Object.keys(node.$tent.attributes).forEach((key)=>clone[key] = node.$tent.attributes[key]);
    if (clone.hasChildNodes()) for (const [index, entry] of clone.childNodes.entries())addAttributes(entry, node.childNodes[index]);
    return clone;
}
const t = [
    "div",
...
const tags = {};
t.forEach((tag)=>tags[tag] = (children, attrs)=>createTag([
            tag,
            children,
            attrs
        ]));
...
},{}]},["d8lhj","jeorp"], "jeorp", "parcelRequiref762")
